...
public class SQLFunc
{
    ...
    public static void Del_DIALOG_DETAIL(string DD_ID)
    {
        var cmd = new SqlCommand();
        cmd.CommandText = @"Delete Dialog_Detail Where DD_ID=@DD_ID";
        cmd.Parameters.Add("DD_ID", SqlDbType.Int).Value = DD_ID;
        Persister.ExecuteNonQuery(cmd);
    }
    ...
    public static void Del_DIALOG_FEEDBACK(string DF_DDID)
    {
        var cmd = new SqlCommand();
        cmd.CommandText = @"Delete Dialog_Feedback Where DF_DDID=@DF_DDID";
        cmd.Parameters.Add("DF_DDID", SqlDbType.Int).Value = DF_DDID;
        Persister.ExecuteNonQuery(cmd);
    }
    ...
    public static DataTable Get_FB_Message_List(string Dialog_Chk)
    {
        var cmd = new SqlCommand();
        cmd.CommandText = @"select top 100 * from FB_message
            where Dialog_Chk=@Dialog_Chk";
        cmd.Parameters.Add("Dialog_Chk", SqlDbType.Int).Value = Dialog_Chk;
        return Persister.ExecuteFB(cmd);
    }
    ...
    public static void Update_FB_Message_Dialog_Chk(string FB_ID)
    {
        var cmd = new SqlCommand();
        cmd.CommandText = @"Update FB_message set Dialog_Chk=1
            where id=@FB_ID";
        cmd.Parameters.Add("FB_ID", SqlDbType.Int).Value = FB_ID;
        Persister.ExecuteFBNonQuery(cmd);
    }
}
